...
def render_growth_structure(growth_df: pd.DataFrame) -> None:
    ...
    total_revenue = df["gross_revenue"].sum() if "gross_revenue" in df.columns else None
    total_orders = df["order_cnt"].sum() if "order_cnt" in df.columns else None
    total_buyers = df["active_buyers"].sum() if "active_buyers" in df.columns else None
    ...
    if {"year_month", "gross_revenue"}.issubset(df.columns):
        ...
        revenue_df = df[["year_month", "gross_revenue"]].copy().set_index("year_month")
    ...
    if "year_month" in df.columns and driver_cols:
        ...
        st.caption(
            "주문 수, 활성 구매자 수, AOV, ARPB를 함께 비교하여 매출 변화가 거래량 요인인지 고객 가치 요인인지 확인합니다."
        )

        driver_df = df[["year_month"] + driver_cols].copy().set_index("year_month")
    ...
    if "year_month" in df.columns and mom_cols:
        ...
        mom_df = df[["year_month"] + mom_cols].copy().set_index("year_month")
