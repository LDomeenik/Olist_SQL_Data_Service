...
def render_growth_drill_down(drill_df: pd.DataFrame) -> None:
    ...
    if not buyer_df.empty:
        ...
        if not base_row.empty and not comp_row.empty:
            ...
            with k1:
                st.metric(
                    label="Revenue",
                    value=f"{_format_number(base_row['gross_revenue'], 'currency')} → {_format_number(comp_row['gross_revenue'], 'currency')}",
            ...
            with k2:
                st.metric(
                    label="Orders",
                    value=f"{_format_number(base_row['order_cnt'])} → {_format_number(comp_row['order_cnt'])}",
            ...
            with k3:
                st.metric(
                    label="Active Buyers",
                    value=f"{_format_number(base_row['buyers'])} → {_format_number(comp_row['buyers'])}",
            ...
            with k4:
                st.metric(
                    label="AOV",
                    value=f"{_format_number(base_row['aov'], 'currency')} → {_format_number(comp_row['aov'], 'currency')}",
    ...
    if not buyer_df.empty:
        buyer_compare = buyer_df[buyer_df["year_month"].isin([growth_month, drop_month])].copy()
    ...
    if not category_df.empty:
        category_compare = category_df[category_df["year_month"].isin([growth_month, drop_month])]
    ...
    if not city_df.empty:
        city_compare = city_df[city_df["year_month"].isin([growth_month, drop_month])]
