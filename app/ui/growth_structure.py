...
#         revenue_df = revenue_df.replace([np.inf, -np.inf], None).fillna(0)
#         revenue_df.index = revenue_df.index.astype(str)

#         try:
#             st.line_chart(revenue_df, use_container_width=True)
#         except Exception:
#             st.warning("매출 차트를 렌더링할 수 없습니다.")
...
#         driver_df = driver_df.replace([np.inf, -np.inf], None).fillna(0)
#         driver_df.index = driver_df.index.astype(str)

#         try:
#             st.line_chart(driver_df, use_container_width=True)
#         except Exception:
#             st.warning("드라이버 차트를 렌더링할 수 없습니다.")
...
#         mom_df = mom_df.replace([np.inf, -np.inf], None).fillna(0)
#         mom_df.index = mom_df.index.astype(str)

#         try:
#             st.line_chart(mom_df, use_container_width=True)
#         except Exception:
#             st.warning("MoM 차트를 렌더링할 수 없습니다.")
...
def render_growth_structure(growth_df: pd.DataFrame) -> None:
    ...
    render_insight_box(
        ...
        message=(
            "매출 변화는 AOV보다 주문 수와 활성 구매자 수 변화에 더 강하게 반응합니다. "
            "즉, 현재 플랫폼은 고객 가치 상승보다 거래량 확장에 의해 성장하는 구조입니다."
        ),
        level="info",
    )
