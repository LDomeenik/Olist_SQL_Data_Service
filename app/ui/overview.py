...
def render_overview(
    ...
    """
    Overview 탭 전체 화면을 렌더링하는 함수

    입력 데이터:
    - growth_df: Growth Structure용 월별 KPI 데이터
    - drill_df: Growth Drill Down 데이터
    - customer_df: Customer Value Structure 데이터
    - ops_df: Operational Stability 데이터

    주요 동작:
    - 전체 핵심 KPI 요약 카드 표시
    - Growth / Operational 핵심 차트 표시
    - 주요 분석 모듈 인사이트 요약 표시
    """
    ...
    if not customer_df.empty and "section_type" in customer_df.columns:
        monthly_customer_df = customer_df[
            customer_df["section_type"] == "monthly_value"
        ].copy()

        if not monthly_customer_df.empty and "year_month" in monthly_customer_df.columns:
            monthly_customer_df = monthly_customer_df.sort_values("year_month").reset_index(
                drop=True
            )
    ...
    total_revenue = (
        ...
        if "gross_revenue" in growth_df.columns
        else None
    )
    total_orders = (
        growth_df["order_cnt"].sum()
        if "order_cnt" in growth_df.columns
        else None
    )
    ...
    repeat_rate = None
    if not customer_df.empty and "section_type" in customer_df.columns:
        new_repeat_df = customer_df[
            customer_df["section_type"] == "new_repeat_share"
        ].copy()
