...
def save_uploaded_raw_files(uploaded_files) -> tuple[list[str], list[str]]:
    """
    업로드된 Raw CSV 파일을 지정된 경로(RAW_DATA_DIR)에 저장하는 함수

    - 파일명 검증 후 허용된 파일만 저장
    - 저장된 파일 목록과 스킵된 파일 목록 반환
    """
...
def get_latest_modified_time(file_names: list[str], base_dir: Path) -> str:
    """
    주어진 파일 목록 중 가장 최근 수정 시간을 반환하는 함수

    - 파일이 하나도 없으면 "No data" 반환
    - UI에서 마지막 업데이트 시각 표시 용도
    """
    latest_time = None
...
def get_dataset_status(
    ...
    상태 종류:
    - not_started: 세션 기준 해당 단계가 아직 실행되지 않음
    - partial: 일부 파일만 존재
    - complete: 모든 파일 존재

    반환:
    (상태 문자열, 파일별 존재 여부 리스트)
    """
...
def render_dataset_expander(
    ...
    """
    Raw / Output 데이터 상태를 사이드바 expander 형태로 렌더링하는 함수
...
def render_sidebar_controls() -> None:
    ...
    raw_time = get_latest_modified_time(RAW_FILE_LIST, RAW_DATA_DIR)
