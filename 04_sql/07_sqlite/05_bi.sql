...
/****************************************************************************************************************************************/
/*
 * Section: vw_growth_structure
 */
/****************************************************************************************************************************************/

DROP VIEW IF EXISTS vw_growth_structure;

CREATE VIEW vw_growth_structure AS
WITH base AS (
    SELECT  year_month
            ,gross_revenue
            ,order_cnt
            ...
            ,repeat_buyers
            ,repeat_buyer_rate
            ,ROUND(gross_revenue * 1.0 / NULLIF(active_buyers, 0), 2) AS arpb
            ,ROUND(order_cnt * 1.0 / NULLIF(active_buyers, 0), 4) AS orders_per_buyer
      FROM  vw_kpi_monthly_core
...
SELECT  year_month
        ...
        ,orders_per_buyer
        ,ROUND(
            (gross_revenue - LAG(gross_revenue) OVER (ORDER BY year_month)) * 1.0
            / NULLIF(LAG(gross_revenue) OVER (ORDER BY year_month), 0)
        , 6) AS mom_gross_revenue
        ,ROUND(
            (order_cnt - LAG(order_cnt) OVER (ORDER BY year_month)) * 1.0
            / NULLIF(LAG(order_cnt) OVER (ORDER BY year_month), 0)
        , 6) AS mom_order_cnt
        ,ROUND(
            (active_buyers - LAG(active_buyers) OVER (ORDER BY year_month)) * 1.0
            / NULLIF(LAG(active_buyers) OVER (ORDER BY year_month), 0)
        , 6) AS mom_active_buyers
        ,ROUND(
            (aov - LAG(aov) OVER (ORDER BY year_month)) * 1.0
            / NULLIF(LAG(aov) OVER (ORDER BY year_month), 0)
        , 6) AS mom_aov
        ,ROUND(
            (arpb - LAG(arpb) OVER (ORDER BY year_month)) * 1.0
            / NULLIF(LAG(arpb) OVER (ORDER BY year_month), 0)
        , 6) AS mom_arpb
        ,ROUND(
            (orders_per_buyer - LAG(orders_per_buyer) OVER (ORDER BY year_month)) * 1.0
            / NULLIF(LAG(orders_per_buyer) OVER (ORDER BY year_month), 0)
        , 6) AS mom_orders_per_buyer
  FROM  base;
...
/****************************************************************************************************************************************/
/*
 * Section: vw_growth_drill_down
 */
/****************************************************************************************************************************************/

DROP VIEW IF EXISTS vw_growth_drill_down;

CREATE VIEW vw_growth_drill_down AS
WITH base_customer AS (
    SELECT  year_month
      ...
      FROM  vw_base_customer_monthly_purchase
...
/****************************************************************************************************************************************/
/*
 * Section: vw_customer_value_structure
 */
/****************************************************************************************************************************************/

DROP VIEW IF EXISTS vw_customer_value_structure;

CREATE VIEW vw_customer_value_structure AS
...
monthly_value AS (
    SELECT  year_month
            ...
            ,SUM(order_cnt) AS order_cnt
            ,ROUND(SUM(gross_revenue) * 1.0 / NULLIF(COUNT(DISTINCT customer_unique_id), 0), 2) AS arpb
            ,ROUND(SUM(order_cnt) * 1.0 / NULLIF(COUNT(DISTINCT customer_unique_id), 0), 4) AS orders_per_buyer
            ,ROUND(SUM(gross_revenue) * 1.0 / NULLIF(SUM(order_cnt), 0), 2) AS aov
      FROM  customer_month
     GROUP
        BY  year_month
),
...
SELECT  'new_repeat_share' AS section_type
        ...
        ,NULL AS aov
        ,ROUND(
            gross_revenue * 1.0 / NULLIF(SUM(gross_revenue) OVER (PARTITION BY year_month), 0)
        , 6) AS revenue_share
        ,NULL AS retention_rate
  FROM  new_repeat
...
SELECT  'cohort_retention' AS section_type
        ...
        ,NULL AS aov
        ,NULL AS revenue_share
        ,ROUND(cohort_active_buyers * 1.0 / NULLIF(cohort_size, 0), 6) AS retention_rate
...
/****************************************************************************************************************************************/
/*
 * Section: vw_operational_stability
 */
/****************************************************************************************************************************************/

DROP VIEW IF EXISTS vw_operational_stability;

CREATE VIEW vw_operational_stability AS
WITH monthly_kpi AS (
    SELECT  year_month
            ,total_orders
            ,canceled_orders
            ...
            ,unavailable_rate
            ,failed_rate
      FROM  vw_kpi_monthly_cancellation
     WHERE  year_month BETWEEN '2017-01' AND '2018-08'
),
status_dist AS (
...
category_cancel_risk AS (
    SELECT  year_month
            ...
            ,ROUND(COUNT(DISTINCT CASE WHEN order_status = 'canceled' THEN order_id END) * 1.0 / NULLIF(COUNT(DISTINCT order_id), 0), 6) AS cancel_rate
            ,ROUND(COUNT(DISTINCT CASE WHEN order_status = 'unavailable' THEN order_id END) * 1.0 / NULLIF(COUNT(DISTINCT order_id), 0), 6) AS unavailable_rate
            ,ROUND(COUNT(DISTINCT CASE WHEN order_status IN ('canceled', 'unavailable') THEN order_id END) * 1.0 / NULLIF(COUNT(DISTINCT order_id), 0), 6) AS failed_rate
      FROM  order_category
     GROUP
        BY  year_month
            ,dimension_value
...
city_cancel_risk AS (
    SELECT  dd.year_month
            ...
            ,ROUND(COUNT(DISTINCT CASE WHEN fo.order_status = 'canceled' THEN fo.order_id END) * 1.0 / NULLIF(COUNT(DISTINCT fo.order_id), 0), 6) AS cancel_rate
            ,ROUND(COUNT(DISTINCT CASE WHEN fo.order_status = 'unavailable' THEN fo.order_id END) * 1.0 / NULLIF(COUNT(DISTINCT fo.order_id), 0), 6) AS unavailable_rate
            ,ROUND(COUNT(DISTINCT CASE WHEN fo.order_status IN ('canceled', 'unavailable') THEN fo.order_id END) * 1.0 / NULLIF(COUNT(DISTINCT fo.order_id), 0), 6) AS failed_rate
      FROM  fact_orders AS fo
     INNER
      JOIN  dim_date AS dd
        ON  dd.date_key = fo.order_purchase_date_key
...
SELECT  'order_status_share' AS row_type
        ...
        ,ROUND(order_cnt * 1.0 / NULLIF(SUM(order_cnt) OVER (PARTITION BY year_month), 0), 6) AS status_order_share
  FROM  status_dist
